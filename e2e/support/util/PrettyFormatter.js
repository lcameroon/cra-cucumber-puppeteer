...
class PrettyFormatter extends SummaryFormatter {
  ...
  logTestCaseName({ sourceLocation }) {
    ...
    this.log(
      `${this.scenarioCount++}) Scenario: ${pickle.name} ${this.colorFns['location'](
  ...
  logTestStep({ testCase, index, result }) {
    ...
    if (pickleStep) {
      this.log(
        `   ${this.colorFns[status](STATUS_CHARACTER_MAPPING[status])} ${gherkinKeyword}${
          pickleStep.text
        }\n`
      );
    }
      ...
      for (const attachment of testStep.attachments) {
        ...
        this.log(
          `     ${this.colorFns['location'](
            `Attachment (${type})${type === 'text/plain' ? ': ' + attachment.data : ''}`
          )}\n`
        );
      }
    }
