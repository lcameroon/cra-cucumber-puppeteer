...
Given(/^the element "([^"]*)" is( not)? visible$/, async function (selector, not) {
...
Given(/^the element "([^"]*)"( does not)? contains? text "([^"]*)"$/, checkContainsText);
...
Given(/^the screenshot of "([^"]*)?" matches the web page?$/, async function (filename) {
