...
Then(/^I expect the element "([^"]*)?" value is( not)? "([^"]*)?"$/, checkElementValue);
...
Then(/^I expect the element "([^"]*)?" is( not)? on the page$/, checkElementExists);
...
Then(/^I expect the cookie "([^"]*)?" value is( not)? "([^"]*)?"$/, checkCookieValue);
...
Then(/^I expect the page title (does not )?contains? "([^"]*)?"$/, checkTitleContains);

Then(/^I expect the page url (does not )?contains? "([^"]*)?"$/, checkUrlContains);

Then(/^I expect the screenshot of "([^"]*)?" matches the web page?$/, async function (
  filename
) {
